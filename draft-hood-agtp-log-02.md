...
abbrev: "AGTP-LOG"
docname: draft-hood-agtp-log-02
category: info
...
informative:
  ...
  AGTP-IDENTIFIERS:
    ...
    date: 2026
  IDP:
    title: "Intent Declaration Protocol"
    author:
      fullname: Tom Sato
    seriesinfo:
      Internet-Draft: draft-sato-soos-idp-00
    date: 2026
  AUDIT-ARCH:
    title: "AI Agent Auditing Architecture"
    author:
...
append-only audit log protocol that underpins the `log-anchored`
Trust Tier 1 verification path defined in {{AGTP}} and provides the
post-hoc audit infrastructure for AGTP-based agent operations.
AGTP-LOG aligns with {{RFC9162}} (Certificate Transparency 2.0) as
...
consistency-proof retrieval, and discovery via the `log_uri` field
...
audited party participates in writing the audit trail.
...
# AGTP-LOG as Audit Infrastructure {#audit-infrastructure}

AGTP-LOG is the audit infrastructure for AGTP-based agent
...
*Audited party cannot forge own trail.* Statements are signed by
...
constraint that requires enforcement.

*Audit records are append-only and tamper-evident.* The verifiable
data structure of {{RFC9162}} guarantees that committed statements
...
tree heads is detectable by monitors observing the log.

*Audit records carry cryptographic provenance.* Receipts per
{{RFC9943}} are independently verifiable: a party holding a
...
*Audit records are bound to wire-level identity.* The
`agtp-subject` field of every statement is a Canonical Agent-ID
...
that must be correlated after the fact.

## Scope: Post-Hoc Audit

AGTP-LOG addresses post-hoc audit: the recording of what has
happened, after it has happened, in a form that can be verified
later. This is the audit problem regulatory frameworks and
compliance systems typically address.

Pre-commitment intent declaration — recording what an agent
...
work; see {{related-audit-work}}.

## Regulatory Context

Audit infrastructure for agent operations is increasingly subject
...
own requirements for agent attribution and audit trails.

AGTP-LOG provides the protocol-layer infrastructure these
...
## Related Audit Work {#related-audit-work}

Multiple parallel efforts in the agent infrastructure space
address adjacent audit concerns. AGTP-LOG positions explicitly
relative to:

*Pre-commitment intent declaration:* {{IDP}} defines intent
...
*HTTP-based audit data models:* Work in progress in the IETF
(e.g., {{AUDIT-ARCH}}) proposes audit data models layered over
...
coexist for the agent traffic each substrate carries.

*Transparency standards (SCITT, RFC 9162):* AGTP-LOG reuses these
verifiable data structures and receipt formats. AGTP-LOG is a
profile and deployment pattern for SCITT-aligned audit
infrastructure specific to AGTP agent identity events.
...
Log Operator:
: The party that operates a log. In this revision the log operator
...
envelope (see {{entry-format}}). The triggering methods are the
...
| `agent-lifecycle-suspended` | DEACTIVATE method completion | Canonical Agent-ID entering Suspended state |
...
The entry format specified in this section is the
...
({{local-lifecycle-stream}}), which **MAY** use either a JWS
or a COSE_Sign1 envelope. AGTP servers that submit lifecycle
events to an AGTP-LOG transparency log **MUST** convert the
...
## Per-Agent Local Lifecycle Stream {#local-lifecycle-stream}

In addition to the public append-only transparency log, AGTP
...
stream is retrievable via `INSPECT target=lifecycle` per
{{AGTP}}.

The local stream **MAY** use either of two envelope formats,
selected by operator policy:

- **JWS Compact** per {{RFC7515}}. The signed envelope is the
  same JWS Compact serialization used for Attribution-Records.
  Each stream line carries a `jws:` prefix followed by the
  JWS Compact form. This is the default format and the
  simplest for operators without SCITT integration.
- **COSE_Sign1** per {{RFC9943}}. The signed envelope is a
  COSE_Sign1 structure matching the statement format in
  {{entry-format}}. Each stream line carries a `cose:` prefix
  followed by the base64url-encoded COSE_Sign1 bytes. This
  format is appropriate for deployments aligning with SCITT.

The two formats **MAY** coexist within a single agent's
...
**MUST** carry the per-entry `format` field defined in
{{AGTP}} so clients can route each entry to the correct
parser.

The Audit-ID for a lifecycle event is the SHA-256 of the
...
Attribution-Record Audit-IDs per {{AGTP-IDENTIFIERS}}.

An AGTP server that operates an AGTP-LOG transparency log
client **MUST** convert local lifecycle entries into
COSE_Sign1 statements conforming to {{entry-format}} before
...
governance-platform-operated public record.

# Security Considerations {#security}

## Audited Party Forgery {#audited-party-forgery}

A common threat model for audit systems is the audited party
forging, altering, or omitting their own audit records. AGTP-LOG
addresses this structurally rather than through policy controls.

Statements in AGTP-LOG are signed by the governance platform that
...
- Forge a statement attributing actions to a different agent
  identity (statements bind to `agtp-subject` and are signed by the
  log operator)
- Omit an event from its own audit trail (commit decisions are made
  by the log operator, not by the agent)
- Alter a committed statement after the fact (the verifiable data
  structure of {{RFC9162}} prevents retroactive modification
  without breaking tree consistency)
- Replace its audit history with a forged sequence (statements are
  identified by position in an append-only log; replacing history
  is detectable by monitors observing the log)

The threat shifts to compromising the governance platform itself
({{log-operator-compromise}}) rather than compromising the agent.
...
agent credentials may be more numerous and shorter-lived.

This structural property distinguishes AGTP-LOG from audit
...
separation is built into the protocol architecture.

## Log Operator Compromise {#log-operator-compromise}

The log operator is also the issuer of statements in the AGTP-LOG
model in this revision. A compromised log operator could:
...
**SHOULD** require witness countersignatures on STHs (when a future revision
...
{: title="AGTP-LOG Event Types Registry"}
...
- **Federation across governance platforms.** Cross-platform agent
  ...
  via another. This revision assumes per-platform isolation.
...
Version 02 is a drift-cleanup revision. The log scope, statement
...
1. **Lifecycle event triggering methods aligned with {{AGTP}}
   v08 eighteen-method floor.** {{AGTP}} v08 promotes five
   ...
   `agent-genesis-issued` by ACTIVATE, `agent-genesis-revoked`
   by REVOKE, `agent-lifecycle-suspended` by DEACTIVATE
   (replacing the v01 SUSPEND triggering method, which is a
   Mechanics in-session pause method, not a lifecycle
   transition), `agent-lifecycle-reinstated` by REINSTATE,
   and `agent-lifecycle-deprecated` by DEPRECATE. The five
   event types themselves are unchanged; only the method-to-
   event mapping is corrected and finalized.

2. **Per-Agent Local Lifecycle Stream section added.** A
   new section ({{local-lifecycle-stream}}) specifies the
   per-agent lifecycle stream maintained by AGTP servers as
   distinct from the public transparency log defined in this
   document. Local streams **MAY** use either JWS Compact
   (with a `jws:` prefix) or COSE_Sign1 (with a `cose:`
   ...
   transparency log **MUST** convert to the COSE_Sign1
   statement format specified in {{entry-format}}. The
   Audit-ID derivation rule covers both envelopes. This
   documents the SCITT-aligned implementation that ships
   with v07/v08 conformant deployments.

2. **Normative reference to {{AGTP}} updated to v08.**
   Informative references to {{AGTP-CERT}} and {{AGTP-TRUST}}
   updated to v01.

## Wire Format Compatibility

None. Statement and receipt formats are unchanged. Logs that
