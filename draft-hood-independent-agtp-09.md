...
abbrev: "AGTP"
docname: draft-hood-independent-agtp-09
category: info
...
informative:
  ...
  AGTP-COMPOSITION:
    ...
    author:
      fullname: Chris Hood
    seriesinfo:
      Internet-Draft: draft-hood-agtp-composition-01
    date: 2026
  AGTP-BINDINGS:
    ...
    author:
      fullname: Chris Hood
    seriesinfo:
      Internet-Draft: draft-hood-agtp-bindings-00
    date: 2026
  AGTP-MERCHANT:
...
specified in {{AGTP-BINDINGS}}. AGTP is designed to be composable
with existing agent frameworks, not to replace them.
...
Owner-ID:
: The identifier of the principal (human, organization, or service) on
  whose behalf an agent operates. Carried in the agent identity document
  referenced by `Agent-ID`; also surfaced as the `Owner-ID` request
  header when explicit wire-layer carriage is needed. The locked
  identifier name throughout the AGTP family is `Owner-ID`; earlier
...
AGTP's design follows five architectural principles that distinguish it
...
are structurally impossible on AGTP, rather than merely mitigated.

The principles are stated abstractly so they apply beyond any specific
example (such as the BadHost vulnerability discussed in
{{related-work-section}}). A reviewer evaluating whether AGTP succeeds
at its design goals should ask whether the protocol satisfies all five
principles, not whether it addresses any single concrete vulnerability.

## Substrate vs Application Layer Separation

AGTP provides substrate primitives. Adjacent layers (Agent Group
...
**MUST NOT** rely on application-layer cooperation for properties it
provides; it **MUST** enforce them structurally at the transport
boundary.

This principle has direct consequences:

- Identity headers (Agent-ID, Owner-ID, Authority-Scope) are wire-
  layer facts the substrate verifies before dispatch to the
  application. They are not application-layer conventions an
  application is expected to honor.

- Authority enforcement happens at the protocol layer before
  application code runs. A scope violation returns 455 from the
  AGTP layer, not from the application's authorization middleware.

- Attribution is produced by the substrate on every response. An
  application cannot opt out of attribution, fail to produce it, or
  produce a malformed attribution that infrastructure cannot read.

The substrate's correctness does not depend on the application
...
structurally, before any middleware runs.

## Identity at the Wire

Agent identity is a wire-layer fact in AGTP, not an application-layer
convention reconstructed from request properties.

Every AGTP request carries Agent-ID, Owner-ID, and Authority-Scope as
...
agent sent a request.

This principle eliminates an entire vulnerability class: bugs where
...
inferred from headers the protocol parses for other purposes.

Vulnerabilities affecting HTTP-based agent infrastructure that depend
...
identity from those properties.

## Heteronomy: Agents Are Created by Humans

Agents are not autonomous in the strong sense. Every agent has a
...
- The Agent Genesis is signed by a registrar before any agent traffic
...
- The Owner-ID identifies the human or organizational principal
  accountable for the agent. The owner is recorded on the Agent
  Genesis at registration, not asserted by the agent at runtime.

- The Authority-Scope is granted by a principal at registration, not
  claimed by the agent at runtime. An agent **MAY** narrow its claimed
  scope on a specific request below its declared scope; an agent
  **MUST NOT** claim scope beyond what its Genesis grants.

- The Agent Certificate is issued by a Certificate Authority that
  ...
  established outside the certificate.

This architectural principle distinguishes AGTP from approaches that
...
The implications for governance, attribution, and accountability are
...
## Structural Enforcement, Not Policy Enforcement

Security properties in AGTP are structural --- they hold because the
...
or middleware checks the right field.

The distinction matters in two ways:

**Vulnerability impossibility vs vulnerability mitigation.** A
...
**Defense in depth vs defense by configuration.** AGTP composes with
...
Concrete example: the BadHost vulnerability class (CVE-2026-48710
...
the architectural pattern that produces the vulnerability.

This principle applies to other vulnerability classes as well: scope
...
addressed structurally rather than through policy enforcement.

## Substrate Carries Its Own Operational Properties

Identity, authority, attribution, delegation chain, trust posture,
and session state are properties of the substrate itself, not
properties layered on top by applications.

This principle enables uniform infrastructure handling regardless of
...
infrastructure at the wire layer.

The principle has economic and operational consequences:

- **One governance layer covers all agent traffic.** Operators do not
  need separate enforcement for each AGMP. The AGTP governance layer
  handles MCP, A2A, ACP, ANP, and any future protocols uniformly.

- **Audit and attribution compose across protocols.** An attribution
  ...
  need protocol-specific parsing.

- **Future-proof composition.** Adjacent layers not yet defined can
  ...
  from substrate properties without negotiating each property anew.

Adjacent layers benefit from substrate properties without needing to
...
# Related Work and Existing Approaches {#related-work-section}
...
QUIC {{RFC9000}}:
: Modern multiplexed transport with reduced connection overhead.
...
in deployments that use one.

AGTP is transport-neutral: AGTP semantics do not depend on which
transport carries them. Transport bindings for TCP/TLS and QUIC are
specified in {{AGTP-BINDINGS}}, including the AGTP-specific replay-
...
Implementations **MUST NOT** rely on transport-level connection
termination as a request-completion signal in addition to
`Content-Length`. Transport-specific framing requirements, including
the prohibition on TLS socket-level half-close and the handling of
QUIC stream termination, are specified in {{AGTP-BINDINGS}}.
Implementations that require an explicit completion signal in
addition to `Content-Length` **MUST** rely on the AGTP session-close
semantics described in {{methods-suspend}}.
...
| Principal-ID | Renamed to `Owner-ID` in line with the locked AGTP taxonomy. The Owner-ID is recorded in the agent identity document referenced by `Agent-ID`; not transmitted as a separate request header. See {{AGTP-IDENTIFIERS}}. |
...
| 401 | Unauthorized | Authentication failure. Covers Agent-ID not recognized or not authenticated, INSPECT read-access ACL failure for unauthenticated callers, lifecycle-method invocation without a verified client certificate when `genesis_issuer` mode is in effect, and external-credential failures (e.g., missing `Authorization` header on a method that requires one, or an `Authorization` header that fails validation). The response body **SHOULD** carry a structured `reason` from the vocabulary `agent-unauthenticated`, `oauth-required`, `oauth-invalid`, `acl-anonymous-blocked`, or `genesis-issuer-cert-required` to disambiguate the failure mode. See {{AGTP-COMPOSITION}} for the External Identity Provider composition profile and the OAuth-specific reasons. |
...
| 462 | Insufficient Budget | Commerce request budget envelope is insufficient for the receiving agent's pricing tier or settlement requirements. AGTP-specific. See {{AGTP-COMMERCE}}. |
...
| `application/vnd.agtp.identity+json` | Agent Identity Document (JSON) | Registered (vendor tree) |
| `application/vnd.agtp.identity+yaml` | Agent Identity Document (YAML) | Registered (vendor tree) |
...
## Detailed Composition Profiles

Detailed normative composition profiles are specified in
{{AGTP-COMPOSITION}}, including:

- **AGMP composition profiles** for MCP, A2A, and ACP, covering
  identity mapping, delegation mapping, session continuity, and
  capability advertisement.

- **External Identity Provider composition** specifying how AGTP
  identity (Agent-ID, Agent Certificate, Authority-Scope) and
  externally-issued credentials (OAuth bearer tokens, OIDC
  `id_token`s, SPIFFE SVIDs, enterprise IdP session tokens) compose
  as orthogonal axes. The three composition patterns (AGTP-only,
  AGTP plus IdP credential, OIDC-federated Genesis trust), the
  `Authorization` header semantics, the 401 reason vocabulary
  (`oauth-required`, `oauth-invalid`), the token-opacity-on-
  Attribution-Record rule, and backward compatibility guidance are
  specified there normatively.

- **HTTP Gateway composition** specifying the translation surface
...
Servers implementing the AGTP base specification and any of these
composition profiles **MUST** follow the mapping rules in
{{AGTP-COMPOSITION}}.
...
- **DISCOVER**: The DISCOVER method defined in {{AGTP-DISCOVERY}} is
...
| 462 | Insufficient Budget | Commerce request budget envelope is insufficient for the receiving agent's pricing tier or settlement requirements. Returned when a caller's Budget-Limit header is below the receiving agent's published price for the requested method/scope, or when the declared settlementMethod does not match any of the receiver's accepted settlement endpoints. Full definition in {{AGTP-COMMERCE}}. | {{AGTP-COMMERCE}} |
...
| `application/vnd.agtp.identity+json` | Agent Identity Document (JSON) | {{identity-document}} | Registered (vendor tree) |
| `application/vnd.agtp.identity+yaml` | Agent Identity Document (YAML) | {{identity-document}} | Registered (vendor tree) |
...
| `application/agtp-pricing+json` | AGTP commerce pricing manifest | {{AGTP-COMMERCE}} | Planned (AGTP-COMMERCE) |
{: title="AGTP Media Type Registrations"}

"Registered (vendor tree)" indicates that IANA registration is
complete in the `application/vnd.*` vendor tree as of the date of
...
publication of the AGTP family.
...
Initial registrations (all Permanent): Agent-ID, Owner-ID, Authority-Scope,
...
AGTP-Catalog-Warning, AGTP-Endpoint-Warning, Allow-RCNS.
...
concurrently. The `Authorization` header semantics for composition
with external identity providers are specified in {{AGTP-COMPOSITION}}.
The `Allow-RCNS` header semantics for caller opt-in to Runtime
Contract Negotiation are specified in {{AGTP-API}}.

Headers reserved for future revisions (Priority, TTL, AGTP-Zone-ID,
...
specifies their normative semantics.

The `Budget-Limit` header is defined in {{AGTP-COMMERCE}} and
registered concurrently with that document.
...
Version 09 is a light-to-moderate refactor that extracts transport
...
`Principal-ID` to `Owner-ID` rename in the wire-header taxonomy, and
...
identity model, and trust model are unchanged.

## Substantive Changes in v09

The following substantive changes were made in v09:

1. **Transport mechanics extracted to {{AGTP-BINDINGS}}.** The base
   ...
   transport-neutral text pointing to {{AGTP-BINDINGS}}; the
   Transport-Layer Alternatives section's QUIC entry is trimmed to
   reference {{AGTP-BINDINGS}} for the binding; the Wire-Format
   Framing section retains the abstract requirement that
   `Content-Length` is the sole signal of message completion but
   ...
   {{AGTP-BINDINGS}}. This responds to review feedback from Akira
   Okutomi on draft-hood-independent-agtp-08.

2. **Detailed composition profile material extracted to
   {{AGTP-COMPOSITION}}.** The HTTP Gateway Sidecar section and the
   Composition with External Identity Providers section, both
   specified normatively in v08, are extracted to {{AGTP-COMPOSITION}}
   ...
   Profiles" provides cross-references to {{AGTP-COMPOSITION}} for
   the extracted material.

3. **Architectural Principles section added.** A new top-level
   section ({{architectural-principles}}) articulates the broader
...
4. **`Principal-ID` to `Owner-ID` rename completed.** The wire
   header definition and the glossary entry for the principal
   identifier are renamed from `Principal-ID` to `Owner-ID`, aligning
   the base specification with the locked AGTP taxonomy established
   in {{AGTP-IDENTIFIERS}}, {{AGTP-CERT}}, and the other companion
   drafts. The Retired Headers table documents the rename. `Owner-ID`
   ...
   break is introduced beyond what {{AGTP-IDENTIFIERS}} already
   specifies.

5. **IANA Considerations updated to reflect completed
   registrations.** The two Agent Identity Document media types
   (`application/vnd.agtp.identity+json` and
   `application/vnd.agtp.identity+yaml`) are marked as "Registered
   (vendor tree)" rather than the prior "Vendor-tree registration
   submitted." The Media Type Registry table also adds the
   `application/agtp-pricing+json` media type from {{AGTP-COMMERCE}}
   for cross-document discoverability. The Header Field Registry
   adds `Owner-ID`, `Authorization`, and `Allow-RCNS` to the initial
   registrations. The reserved-headers list removes `Budget-Limit`
   (now defined in {{AGTP-COMMERCE}}).

6. **Informative references updated to current companion draft
   versions and new companion drafts added.** References updated:
   {{AGTP-CERT}} now points at draft-hood-agtp-agent-cert-03 (was
   v01); {{AGTP-TRUST}} now points at draft-hood-agtp-trust-02 (was
   v01); {{AGTP-DISCOVERY}} renamed from `AGTP-DISCOVER` and points
   at draft-hood-agtp-discovery-01 (was v00); {{AGTP-COMPOSITION}}
   updated to v01 with new title reflecting expanded scope.
   References added: {{AGTP-BINDINGS}} (the new transport bindings
   draft); {{AGTP-PRESENCE}} (ambient discovery); {{AGTP-LEI}}
   (institutional identity); {{AGTP-COMMERCE}} (open commerce
   specification).

## Architectural Direction

The v09 refactor follows a deliberate strategy of incremental
...
companion draft family matures.

## Wire Format Compatibility

The v09 changes are framing additions, cross-reference updates, and
identifier-taxonomy alignment. The wire format defined in
{{wire-format}}, the request and response line grammars, the header
...
servers and clients, subject to the `Principal-ID` to `Owner-ID`
rename per {{AGTP-IDENTIFIERS}}.
...
16. **Method aliases and HTTP gateway sidecar.** The Method
    ...
    extracted to {{AGTP-COMPOSITION}} as the HTTP Gateway
    composition profile.)
...
: The identifier of the principal (human, organization, or service) on
  ...
  referenced by `Agent-ID`; also surfaced as the `Owner-ID` request
  header when explicit wire-layer carriage is needed. The locked
  identifier name throughout the AGTP family.
