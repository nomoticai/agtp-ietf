...
abbrev: "AGTP-TRUST"
docname: draft-hood-agtp-trust-02
category: info
...
- **Credit Rating Analogy and Composable Trust Providers**: the
...
- **Trust Score**: the runtime behavioral assessment overlaid on
  ...
  scope admission and visibility enforcement.
...
# Credit Rating Analogy and Composable Trust Providers {#credit-rating}

The architectural pattern this document specifies is structurally
...
providers in the agent ecosystem.

## The Credit Rating Pattern

Credit bureaus including Experian, Equifax, TransUnion, and similar
institutions in other jurisdictions operate on a consistent pattern:

- The underlying entity (a person or business) has identifiers
  ...
  registries).
- The credit bureau collects behavioral evidence about the entity
  from many independent sources (creditors, public records, payment
  histories, employment records).
- The bureau computes a credit score using a proprietary algorithm
  that aggregates and weights the evidence.
- The bureau signs and timestamps the score, making it consumable by
  third parties for credit decisions.
- Multiple bureaus may score the same entity differently; consumers
  of credit scores select which bureau to trust for which decisions.

The credit scoring algorithms used by FICO, VantageScore, and the
...
which scores are exchanged.

## AGTP-TRUST as Substrate for Agent Credit Scoring

AGTP-TRUST applies this pattern to agent infrastructure. The
architectural decomposition is:

| Layer | Human Credit Scoring | AGTP Trust Scoring |
|-------|----------------------|---------------------|
| Identifier | SSN, EIN, national IDs | Canonical Agent-ID, Owner-ID |
| Evidence | Payment history, public records, creditor reports | Attribution-Records, AGTP-LOG, peer interactions, attestations |
| Score Computation | Proprietary algorithm (FICO, VantageScore, bureau-specific) | Proprietary algorithm (per trust provider) |
| Score Format | Numeric score (300-850 for FICO) with bureau signature | `trust_score` on [0.0, 1.0] with issuer signature per this document |
| Consumer Decision | Lender uses score for credit approval | AGTP infrastructure uses score for authority decisions |
| Multiple Providers | Person has scores from multiple bureaus | Agent has scores from multiple trust providers |

AGTP-TRUST normatively defines the identifier infrastructure, the
...
## Composable Trust Providers

This document explicitly invites existing credit bureaus and new
...
credit scoring transfers naturally to agent trust scoring:

- Data aggregation pipelines that ingest behavioral evidence at scale
- Algorithmic scoring infrastructure with operational maturity
- Regulatory compliance frameworks for fair, accurate, and disputable
  scoring decisions
- Distribution channels through which scores reach consumers
- Dispute resolution and correction processes
- Multi-bureau coexistence patterns that allow consumers to source
  from multiple providers

These institutions are positioned to extend their existing
...
New entrants specializing in agent-specific trust scoring are equally
...
## Where the Credit Scoring Lives

A specific architectural point: agent credit scoring infrastructure
...
This composition matters because it lowers the adoption barrier for
...
application-layer scoring compose cleanly.

## Forward-Looking Composability

This document is intentionally forward-looking about composable trust
...
to extend to agents; new entrants will compete for niches.

AGTP-TRUST is designed to accommodate both incumbent and new-entrant
...
operational ecosystems are expected to layer on top.
...
## Agent Identity Document Trust Posture Loading {#trust-posture-loading}

The Agent Identity Document carries the trust-posture fields
`trust_tier`, `verification_path`, and `trust_warning` (as
defined in {{AGTP}}). The fields **MAY** be declared directly
in the Agent Identity Document, **MAY** be derived from the
agent's paired Agent Genesis at load time, or **MAY** fall
through to conservative defaults.

A conforming AGTP server **MUST** resolve trust-posture fields
on the Agent Identity Document according to the following
precedence, evaluated independently for each field:

1. **Explicit declaration.** If the field is present in the
   source Agent Identity Document JSON, that value is
   authoritative for the field and **MUST NOT** be overridden
   by a Genesis-derived value. Operator intent is preserved.

2. **Genesis-derived fallback.** If the field is absent from
   the Agent Identity Document and a paired Agent Genesis is
   loaded for the agent, the server **MUST** lift the value
   from the Genesis: `trust_tier` from the Genesis's
   `trust_tier` field, `verification_path` from the Genesis's
   `verification_path` field. The lifted value is treated as
   authoritative for the lifetime of the loaded document.

3. **Conservative default.** If the field is absent and no
   paired Agent Genesis is loaded, the server **MUST**
   default the agent to Trust Tier 2 with
   `verification_path: org-asserted`. This is the
   conservative posture: an unverified agent is treated as
   org-asserted rather than as Tier 1 or Tier 3.

The `trust_warning` field is computed after the precedence is
resolved, on the basis of the resolved `trust_tier`:
...
| 2 | If the operator did not set a warning, the server **MUST** auto-populate `trust_warning: "verification-incomplete"` per {{tier-summary}}. Operator-set values are preserved. |
| 3 | Omitted unless the operator explicitly set a warning. Tier 3 is experimental-only; warning policy is operator-controlled. |
{: title="trust_warning Auto-Population"}

The `owner_id` field on the Agent Identity Document follows
the same precedence as the trust-posture fields (explicit
declaration > Genesis-derived > absent). `owner_id` is not a
trust-posture concept and its semantics are specified by
{{AGTP-IDENTIFIERS}}; the loading rule is included here
because the Agent Genesis is the common source of all four
fields and operators benefit from a single precedence rule.

A server **SHOULD** log at startup which fields it resolved
by Genesis-derived fallback and which by conservative
default, so operators can audit whether their declared
posture matches what the server resolved.

### Surfacing Trust Posture on Response Headers {#trust-response-headers}

A conforming AGTP server **SHOULD** stamp the resolved trust
posture on every response by emitting the following response
headers defined in {{AGTP}}:
...
The headers carry values resolved by the precedence rule in
{{trust-posture-loading}}. Servers that have not resolved a
trust posture for the responding agent (e.g., transport-only
deployments without a loaded Agent Identity Document) **MAY**
omit the headers.

Relying parties **MAY** branch on the response headers to apply
trust-tier-conditional policy on every exchange without
consulting the Agent Identity Document. Relying parties **MUST
NOT** treat the absence of `Trust-Warning` on a Tier 2 response
...
set on the Agent Identity Document.
...
## Trust Anchor Resolution {#trust-anchor-resolution}

Tier 1 verification requires the relying party to determine
that a Genesis-issuer key is trustworthy. The mechanism by
which a verifier decides this is a deployment-policy
concern, but two patterns are defined here normatively:

**Local trust anchors.** A verifier maintains a static list
of trusted Genesis-issuer keys (typically operator-curated
per governance zone). Each entry has the form
`{"type": "key", "value": KEY}` where KEY is the
base64url-encoded Ed25519 public key. A Genesis
whose `issuer_public_key` matches an entry in the list is
...
**OIDC-federated trust anchors.** A verifier maintains a
list of OIDC discovery anchors. Each entry has the form
`{"type": "oidc", "discovery_url": URL, "trusted_issuer": ISSUER}`
...
locates the `jwks_uri`, fetches the JWKS, and confirms that
the Genesis-issuer `issuer_public_key` matches one of the
keys published in the JWKS. The JWKS response **SHOULD** be
cached with a reasonable TTL (default 1 hour); cache misses
**MUST NOT** propagate as Tier 1 trust failures —
unresolvable issuers fall back to lower-tier treatment per
local policy.

The OIDC-federated path lets enterprise deployments bottom
...
The two pattern types are not mutually exclusive: a
verifier **MAY** maintain a mixed list with both `key` and
`oidc` entries and consider an issuer trusted if any entry
matches.

Verifiers **MUST** treat network failures on OIDC discovery
...
## Consumption by AGTP-Presence

AGTP-Presence {{AGTP-PRESENCE}} consumes trust posture from this
document in several distinct ways. AGTP-Presence overlay coordinators
and participating agents **MUST** evaluate trust posture as specified
here when performing the operations described below.

### Overlay Admission

Trust-tier-partitioned overlays admit agents based on the agent's
declared and verified trust tier. An overlay coordinator for a Tier 1
Verified overlay **MUST** verify the joining agent's tier assignment
...
1 membership without a verifiable verification path **MUST** be
denied admission.

### Visibility Posture as a Trust-Bearing Claim

The visibility posture declared in an agent's AGTP-CERT extension
per {{AGTP-PRESENCE}} composes with trust tier verification. An
agent declaring `presenceMode: tier-scoped` in its certificate is
...
Consumers evaluating presence claims **MUST** verify that the
...
1 overlay is presenting a contradiction and **MUST** be rejected.

### Trust Score in Scope Admission

Overlay coordinators **MAY** apply trust score thresholds for
...
admission. Agents below the threshold **MAY** be admitted to a less
restricted overlay representing the same capability but **MUST NOT**
be admitted to the threshold-gated overlay.

### Cross-Scope Resolution Trust Evaluation

When a cross-scope resolution per {{AGTP-PRESENCE}} returns an agent
record, the requesting agent **SHOULD** evaluate the returned agent's
...
to the cross-scope query response.

### Presence Operations as Behavioral Evidence

ANNOUNCE, WITHDRAW, and PROBE operations performed by an agent
generate behavioral evidence that **MAY** be incorporated into the
agent's `behavioral_history` dimension. Excessive churn, malformed
...
evidence **SHOULD** include presence operations in their evidence
...
Version 02 adds the credit-rating-analogous framing and the
...
unchanged. New material is additive.

## Substantive Changes in v02

The following substantive changes were made in v02:

1. **Credit Rating Analogy section added.** A new section
   ({{credit-rating}}) frames AGTP-TRUST in terms of the credit
...
2. **AGTP-Presence consumption subsection added.** A new subsection
   ...
   operations as behavioral evidence.

3. **Introduction updated to four-part structure.** The introduction
   ...
   and Trust Score parts retain their roles unchanged.

4. **Informative reference to AGTP-PRESENCE added.** Cross-references
   between this document and the AGTP-Presence specification.

## Changes from v00 (Carried Forward from v01)

Version 01 is a drift-cleanup revision. The trust tiers,
...
1. **Trust Posture Loading rule added.** A new normative
   section ({{trust-posture-loading}}) specifies how a server
   resolves the trust-posture fields (`trust_tier`,
   `verification_path`, `trust_warning`) on the Agent
   ...
   beats a conservative Tier 2 / `org-asserted` default. The
   `trust_warning` field is auto-populated by the server when
   the resolved `trust_tier` is `2` and the operator did not
   set a warning. Operator-set values are always preserved.
   The `owner_id` field follows the same precedence and is
   noted alongside, with semantics deferred to
   {{AGTP-IDENTIFIERS}}. This section documents behavior that
   v07-conformant implementations have shipped.

2. **Trust posture surfaced on response headers.** A new
   subsection ({{trust-response-headers}}) documents the
   `Trust-Tier`, `Verification-Path`, and `Trust-Warning`
   response headers defined in {{AGTP}} v08. Conforming
   ...
   after the trust-posture loading rule shipped.

3. **Normative reference to {{AGTP}} updated to v08.**
   Informative references to {{AGTP-CERT}} and {{AGTP-LOG}}
   updated to v01. Informative reference to
   {{AGTP-IDENTIFIERS}} added; the identifier stack is
   referenced from the new trust-posture loading section.

4. **Trust Anchor Resolution section added.** A new section
   ({{trust-anchor-resolution}}) specifies two normative
   ...
   The two patterns compose: a verifier **MAY** maintain a
   ...
   **MUST** be non-fatal: the affected Genesis is treated
   as unresolvable rather than as a verification failure.

## Wire Format Compatibility

None. The v02 additions are framing (credit rating analogy) and
...
continue to interoperate with v02.
