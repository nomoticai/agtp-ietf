...
abbrev: "AGTP-CERT"
docname: draft-hood-agtp-agent-cert-01
category: info
...
: The canonical AGTP Agent-ID bound to this certificate. The
  ...
  the value carried in `subject-agent-id`; the equality held
  ...
  `subject-agent-id`. The relying party **MUST** additionally
...
: The identifier of the human or organizational principal
  ...
  existence. It is distinct from the `acting_principal_id`
...
`subject-agent-id` and **MAY** carry different public keys.
Infrastructure **MUST** use the `subject-agent-id` extension
...
The `subject-agent-id` extension carries the canonical
...
2. **`subject-agent-id` decoupled from certificate public
   ...
   TLS layer must refuse certificates whose `subject-agent-id`
   ...
   `subject-agent-id` extension is authoritative when present;
   ...
   agent **MUST** carry the same `subject-agent-id` and **MAY**
